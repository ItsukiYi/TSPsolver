...
# Pure-PyTorch sparse aggregation (replaces torch_sparse, avoids CUDA version issues)
# Uses torch.scatter operations which are built into PyTorch and always compatible.

def _sparse_aggregate(edge_index, values, num_nodes, mode='sum'):
    """Aggregate sparse edge features to node features.

    Args:
        edge_index: (2, E) tensor of (src, dst) indices
        values: (E, H) tensor of edge features
        num_nodes: int, number of target nodes
        mode: 'sum', 'mean', or 'max'
    Returns:
        (num_nodes, H) aggregated node features
    """
    ...
    if mode == 'sum':
        ...
        deg = torch.zeros(num_nodes, device=values.device, dtype=values.dtype)
        deg.scatter_add_(0, src, torch.ones_like(src, dtype=values.dtype))
        deg = deg.clamp(min=1).unsqueeze(-1)
        return out / deg
    elif mode == 'max':
        # Use scatter_reduce with 'amax' (PyTorch 1.11+)
        return torch.zeros(num_nodes, H, device=values.device, dtype=values.dtype).scatter_reduce(
            0, src.unsqueeze(-1).expand(-1, H), values, reduce='amax', include_self=False)
    else:
        raise ValueError(f"Unknown sparse aggregation mode: {mode}")

_has_torch_sparse = True  # We provide our own implementation
SparseTensor = None
sparse_sum = None
sparse_mean = None
sparse_max = None
...
class GNNLayer(nn.Module):
  ...
  def aggregate(self, Vh, graph, gates, mode=None, edge_index=None, sparse=False):
    ...
    if not sparse:
      ...
      # Pure-PyTorch sparse aggregation (no torch_sparse dependency)
      agg_mode = mode or self.aggregation
      return _sparse_aggregate(edge_index, Vh, graph.size(0), mode=agg_mode)
...
class GNNEncoder(nn.Module):
  ...
  def sparse_encoding(self, x, e, edge_index, time_emb):
    # Pure-PyTorch: we only need num_nodes, not a full SparseTensor
    num_nodes = x.shape[0]
    # Lightweight "adjacency" object carrying just size info
    adj_matrix = type('obj', (object,), {'size': lambda self, dim: num_nodes})()
